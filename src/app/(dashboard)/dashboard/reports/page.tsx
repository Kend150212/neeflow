...
export default function InsightsPage() {
    ...
    return (
        <div className="flex flex-col gap-0 h-full overflow-hidden">
            ...
            {/* ── Tab bar ── */}
            <div className="flex items-center gap-0 border-b px-4 overflow-x-auto shrink-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {/* Overview tab */}
                <button
                    type="button"
                    onClick={() => setActiveTab('overview')}
                    className={`flex items-center gap-1.5 px-3 py-2.5 text-xs font-medium border-b-2 transition-colors whitespace-nowrap cursor-pointer ${activeTab === 'overview' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                >
