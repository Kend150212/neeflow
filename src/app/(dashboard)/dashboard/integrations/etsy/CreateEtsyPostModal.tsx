...
// ── Main modal ────────────────────────────────────────────────────────────────
export default function CreateEtsyPostModal({ open, onClose, products }: Props) {
    ...
    async function handleCreate() {
        if (!activeChannelId) { toast.error(t('integrations.etsy.modal.noChannelError')); return }
        if (selectedPlatforms.size === 0) { toast.error(t('integrations.etsy.modal.noPlatformError')); return }
        ...
        try {
            if (isSingle) {
                ...
                const res = await fetch('/api/posts/generate-from-etsy', {
            ...
            bulkGen.start(cappedProducts.length, 'Etsy Products')
            setStep('starting')
            ...
            if (bulkGen.isStopped()) {
                bulkGen.stop()
                toast.info(t('integrations.etsy.modal.stoppedToast').replace('{created}', String(created)).replace('{total}', String(cappedProducts.length)))
            } else {
                bulkGen.finish()
                toast.success(t('integrations.etsy.modal.doneToast').replace('{created}', String(created)), { duration: 5000 })
            }
        } catch (err) {
            toast.error(err instanceof Error ? err.message : t('integrations.etsy.modal.errorToast'))
            setStep('config')
        }
    }
    ...
    const quotaLabel = imageQuota.limit > 0 ? t('integrations.etsy.modal.quotaUsed').replace('{used}', String(imageQuota.used)).replace('{limit}', String(imageQuota.limit)) : null
    ...
    const WIZARD_STEPS = [
        { id: 1 as const, label: t('integrations.etsy.modal.step1Label') || 'Platforms' },
        { id: 2 as const, label: t('integrations.etsy.modal.step2Label') || 'Media' },
        { id: 3 as const, label: t('integrations.etsy.modal.step3Label') || 'Settings' },
        { id: 4 as const, label: t('integrations.etsy.modal.step4Label') || 'Schedule' },
    ]
            ...
            <DialogContent className="bg-background/98 backdrop-blur border border-border/60 shadow-2xl overflow-hidden p-0 flex flex-col" style={{ width: 'min(98vw, 1400px)', maxWidth: '98vw', height: '95vh', maxHeight: '95vh' }}>
                ...
                <div className="flex items-start justify-between px-5 pt-5 pb-3 border-b shrink-0">
                    <div className="space-y-0.5">
                        <DialogTitle className="flex items-center gap-2 text-base font-semibold">
                            ...
                            {t('integrations.etsy.modal.title')}
                        </DialogTitle>
                        <DialogDescription className="text-xs text-muted-foreground">
                            {cappedProducts.length > 1
                                ? t('integrations.etsy.modal.generateFromPlural').replace('{count}', String(cappedProducts.length))
                                : t('integrations.etsy.modal.generateFrom').replace('{count}', String(cappedProducts.length))}
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                            <span className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20">
                                <Sparkles className="h-2.5 w-2.5" /> {t('integrations.etsy.modal.kbBadge')}
                            </span>
                            {isCapped && (
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4">
                    ...
                    {step === 'config' && (
                        <div className="mb-4">
                            {wizardStep === 1 && (
                                <p className="text-xs text-muted-foreground">{t('integrations.etsy.modal.step1Desc')}</p>
                            )}
                            {wizardStep === 2 && (
                                <p className="text-xs text-muted-foreground">{t('integrations.etsy.modal.step2Desc')}</p>
                            )}
                            {wizardStep === 3 && (
                                <p className="text-xs text-muted-foreground">{t('integrations.etsy.modal.step3Desc')}</p>
                            )}
                            {wizardStep === 4 && (
                                <p className="text-xs text-muted-foreground">{t('integrations.etsy.modal.step4Desc')}</p>
                            )}
                        </div>
                    )}
                        ...
                        <div className="space-y-5">
                            {isSingle && (
                                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-primary/10 border border-primary/20 text-xs text-primary">
                                    ...
                                    {t('integrations.etsy.modal.willOpenCompose')}
                                </div>
                            )}

                            {/* PLATFORMS */}
                            <div className="space-y-2.5">
                                <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{t('integrations.etsy.modal.platforms')}</p>
                                {availablePlatforms.length === 0 ? (
                                    <div className="flex items-center gap-2 text-xs text-muted-foreground py-2">
                                        <Loader2 className="h-3.5 w-3.5 animate-spin" /> {t('integrations.etsy.modal.loadingPlatforms')}
                                    </div>
                                ) : (
                                ...
                                <p className="text-[10px] text-muted-foreground">{t('integrations.etsy.modal.selectedCount').replace('{count}', String(selectedPlatforms.size))}</p>
                            </div>

                            {/* TONE */}
                            <div className="space-y-2">
                                <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{t('integrations.etsy.modal.tone')}</p>
                            ...
                            <div className="space-y-3 flex flex-col">
                                <div className="flex items-center justify-between">
                                    <p className="text-sm font-semibold flex items-center gap-2">
                                        ...
                                        {t('integrations.etsy.modal.importImages')}
                                    </p>
                            ...
                            <div className="space-y-3 flex flex-col">
                                <div className="flex items-center justify-between">
                                    <p className="text-sm font-semibold flex items-center gap-2">
                                        ...
                                        {t('integrations.etsy.modal.aiImage')}
                                    ...
                                    <div className="space-y-3 rounded-xl border border-border/60 bg-card/40 p-4 flex-1">
                                        ...
                                        <div className="relative">
                                            ...
                                            <button type="button" onClick={() => setProviderDropOpen(!providerDropOpen)}
                                                ...
                                                <span>{selProvider ? selProvider.name : t('integrations.etsy.modal.selectProvider')}</span>
                                        ...
                                        {imageProvider && (
                                            <div className="relative">
                                                ...
                                                <button type="button" onClick={() => setModelDropOpen(!modelDropOpen)}
                                                    className="w-full flex items-center justify-between rounded-lg border border-border/60 bg-background px-3 py-2 text-xs cursor-pointer hover:border-border">
                                                    <span>{loadingModels ? t('integrations.etsy.modal.loadingModels') : (selModel?.name || imageModel || t('integrations.etsy.modal.selectModel'))}</span>
                                        ...
                                        <div>
                                            ...
                                            <textarea value={imagePrompt} onChange={e => setImagePrompt(e.target.value)}
                                                placeholder={t('integrations.etsy.modal.imagePlaceholder')}
                                            ...
                                            <div className="space-y-2 border-t pt-3">
                                                {img2imgCompatible ? (
                                                    ...
                                                    <>
                                                        <div className="flex items-center justify-between">
                                                            <div className="space-y-0.5">
                                                                <p className="text-xs font-medium">{t('integrations.etsy.modal.useProductRef')}</p>
                                ...
                                {postMode === 'schedule' && !isSingle && (
                                    <div className="space-y-2 pt-2 border-t border-border/40">
                                        <p className="text-[10px] text-muted-foreground">
                                            {t('integrations.etsy.modal.scheduleDesc').replace('{count}', String(cappedProducts.length)).replace('{tz}', channelTimezone)}
                                        </p>
                                        <div className="grid grid-cols-2 gap-2">
                                            <div>
                                                <p className="text-[10px] text-muted-foreground mb-1">{t('integrations.etsy.modal.fromDate')}</p>
                                            ...
                                            <div>
                                                <p className="text-[10px] text-muted-foreground mb-1">{t('integrations.etsy.modal.toDate')}</p>
                            ...
                            <div className="space-y-2 rounded-xl border border-border/60 bg-card/40 p-4">
                                <div>
                                    <p className="text-sm font-semibold flex items-center gap-1.5">
                                        <Check className="h-4 w-4 text-primary" />{t('integrations.etsy.modal.approval')}
                                    </p>
                                    <p className="text-[10px] text-muted-foreground mt-0.5">
                                ...
                                <div className="border-t border-border/40 pt-3">
                                    ...
                                    {approvalMode === 'optional' && (
                                        <div className="flex items-center justify-between">
                                            <div className="space-y-0.5">
                                                <p className="text-xs font-medium">{t('integrations.etsy.modal.requestApproval')}</p>
                                                <p className="text-[10px] text-muted-foreground">{t('integrations.etsy.modal.requestApprovalDesc')}</p>
                                            </div>
                                            <button type="button"
                                    ...
                                    {approvalMode === 'required' && (
                                        <div className="flex items-center justify-between rounded-lg border border-orange-500/30 bg-orange-500/5 px-3 py-2.5">
                                            <div className="space-y-0.5">
                                                <p className="text-xs font-medium text-orange-400">{t('integrations.etsy.modal.approvalRequired')}</p>
                                                <p className="text-[10px] text-muted-foreground">{t('integrations.etsy.modal.approvalRequiredDesc')}</p>
                                            </div>
                    ...
                    {step === 'generating' && (
                        <div className="py-10 flex flex-col items-center gap-4">
                            ...
                            <div className="text-center space-y-1">
                                <p className="font-semibold text-sm">{t('integrations.etsy.modal.generating')}</p>
                    ...
                    {step === 'starting' && (
                        <div className="py-8 flex flex-col items-center gap-5 text-center">
                            ...
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20">
                                ...
                                <span className="text-xs font-bold text-primary">{t('integrations.etsy.modal.postsProgress').replace('{count}', String(cappedProducts.length)).replace('{pct}', String(pct))}</span>
                            </div>
                            <div className="space-y-1.5 px-2">
                                <p className="font-bold text-base">{t('integrations.etsy.modal.bulkStarted')}</p>
                                <p className="text-xs text-muted-foreground leading-relaxed">
                                    {t('integrations.etsy.modal.bulkDesc').replace('{count}', String(cappedProducts.length))}
                                </p>
                            </div>
                            <Button variant="outline" size="sm" className="w-full mt-1" onClick={onClose}>
                                <X className="h-3.5 w-3.5 mr-1" /> {t('integrations.etsy.modal.closeAndContinue')}
                            </Button>
                        </div>
                    )}
                </div>
                    ...
                    <div className="flex items-center gap-2 px-5 py-3 border-t shrink-0">
                        {wizardStep > 1 ? (
                            ...
                            <Button variant="outline" size="sm" className="flex-1" onClick={onClose}>
                                <X className="h-3.5 w-3.5 mr-1" /> {t('integrations.etsy.modal.cancel')}
                            </Button>
                        )}
                        {wizardStep < 4 ? (
                            <Button
                            ...
                            <Button size="sm" className="flex-1 font-semibold" onClick={handleCreate} disabled={selectedPlatforms.size === 0}>
                                ...
                                {isSingle ? t('integrations.etsy.modal.generateAndEdit') : t('integrations.etsy.modal.createDrafts').replace('{count}', String(cappedProducts.length))}
                            </Button>
                        )}
                    </div>
                )}
