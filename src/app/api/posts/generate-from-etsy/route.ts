...
async function importImageToMedia(imageUrl: string, channelId: string): Promise<string | null> {
    try {
        ...
        console.warn('[generate-from-etsy] Failed to import image:', imageUrl, err)
        return null
    }
}
...
async function generateAiImageForPost(
    ...
    try {
        ...
        if (!keyResult.ok) {
            console.warn('[generate-from-etsy] resolveImageAIKey failed:', keyResult)
            return null
        }
        ...
        console.warn('[generate-from-etsy] AI image generation failed:', err)
        return null
    }
}
...
async function generateImageDirectly(
    ...
    try {
        ...
        console.error('[generate-from-etsy] Provider call failed:', err instanceof Error ? err.message : err)
        return null
    }
    ...
    try {
        ...
        const useR2 = await isR2Configured()
        if (!useR2) {
            console.error('[generate-from-etsy] R2 not configured, cannot upload AI image')
            return null
        }
        ...
        console.error('[generate-from-etsy] Image upload to R2 failed:', err instanceof Error ? err.message : err)
    } finally {
        fs.unlink(tmpPath, () => { })
    }
    return null
...
/**
 * POST /api/posts/generate-from-etsy
...
${allHashtags.length > 0 ? `Include 2-5 relevant hashtags from the available list in each version.` : ''}`
        ...
        if (platformList.length > 0) {
            ...
            if (platformStatusRows.length > 0) {
                ...
                await (prisma.postPlatformStatus as any).createMany({
                    ...
                    skipDuplicates: true,
                }).catch((err: unknown) => console.warn('[generate-from-etsy] Could not create platform statuses:', err))
            }
        }
        ...
        if (productId) {
            ...
            }).catch(err => console.warn('[generate-from-etsy] Could not stamp product:', err))
        }
        ...
        console.error('[generate-from-etsy]', err)
