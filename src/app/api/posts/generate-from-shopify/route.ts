...
async function importImageToMedia(imageUrl: string, channelId: string): Promise<string | null> {
    try {
        ...
        console.warn('[generate-from-shopify] Failed to import image:', imageUrl, err)
        return null
    }
}
...
async function generateAiImageForPost(
    ...
    try {
        ...
        if (!keyResult.ok) {
            console.warn('[generate-from-shopify] resolveImageAIKey failed:', keyResult)
            return null
        }
        ...
        console.warn('[generate-from-shopify] AI image generation failed:', err)
        return null
    }
}
...
async function generateImageDirectly(
    ...
    try {
        ...
        console.error('[generate-from-shopify] Provider call failed:', err instanceof Error ? err.message : err)
        return null
    }
    ...
    try {
        ...
        const useR2 = await isR2Configured()
        if (!useR2) {
            console.error('[generate-from-shopify] R2 not configured, cannot upload AI image')
            return null
        }
        ...
        console.error('[generate-from-shopify] Image upload to R2 failed:', err instanceof Error ? err.message : err)
    } finally {
        fs.unlink(tmpPath, () => { })
    }
    return null
...
/**
 * POST /api/posts/generate-from-shopify
...
${allHashtags.length > 0 ? `Include 2-5 relevant hashtags from the available list in each version.` : ''}`
        ...
        console.error('[generate-from-shopify]', err)
