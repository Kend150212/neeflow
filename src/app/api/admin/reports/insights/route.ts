...
async function fetchThreadsInsights(channelPlatform: {
    ...
    try {
        ...
        // If account-level insights returned zeros (threads_manage_insights not yet approved),
        // fall back to summing per-post metrics so the Overview still shows real data.
        if (totalViews === 0 && totalLikes === 0 && threadsPosts.length > 0) {
